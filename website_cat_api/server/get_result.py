...
def keyWordsOfCluster(clusterNo):
  ...
  cursor = conn.execute("SELECT c"+str(clusterNo)+" from KEYWORDS where date_p='"+str(date.today() - timedelta(days = 14))+"'")
  for row in cursor:
...
def checkUrlInDb(url):
  try:
    ...
    for i in range(50):
      query += 'emb_d' + str(i)
      if(i != 49): query += ', '
    query += ' FROM global_data where url="'+str(url)+'"'
    conn = sqlite3.connect("server/database/web.db") 
    cur = conn.cursor()
    cursor = cur.execute(query)
    for row in cursor:
...
def giveUrlInfo(url):
  try:
      ...
      if(len(urlInfo['new_url_vector'])!=0):
        ...
        print("status",status)
        if(status=='fail'):
          return dict({"status":"failure","status_code":content_dict['content']})
      ...
      return dict({"cluster_no":cluster_no,"urlvector":new_url_vector,'status':"200 ok"})
  except Exception as e:
...
def finalFunction(url,nearCond):
  try:
    urlInfo=giveUrlInfo(str(url))
    if(urlInfo['status']=="failure"):
     return json.dumps({"keywords":[],"websites":[],"status":str(urlInfo['status_code'])})
    print("Url Info Done")
    ...
    if(nearCond==0):
      data_set = {"keywords": keywords,"websites":[],'status':status,"cluster_no":str(ClusterNo+1)}
    ...
    data_set = {"keywords": keywords, "websites": nearWebsites,'status':status,"cluster_no":str(ClusterNo+1)}
